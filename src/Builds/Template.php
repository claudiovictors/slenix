...
class Template
{
    ...
    {
        return htmlspecialchars($this->section[$section] ?? $default, ENT_QUOTES, 'UTF-8');
    }
    ...
    private function compile(string $content): string
    {
        $patterns = [
            ...
            '/@csrf/' => '<?php if (isset($_SESSION["_csrf_token"])): ?><input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($_SESSION["_csrf_token"], ENT_QUOTES, \'UTF-8\') ?>"><meta name="csrf-token" content="<?= htmlspecialchars($_SESSION["_csrf_token"], ENT_QUOTES, \'UTF-8\') ?>"><?php endif; ?>',
